<aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'"
    ...
    <nav class="px-2 text-gray-300">
        ...
        <!-- Menu Lihat Data untuk Owner -->
        @if (auth()->user()->role == 'owner')
            <a href="{{ route('admin.products.index') }}"
                class="flex items-center px-4 py-2 mt-2 rounded-md hover:bg-gray-700 {{ request()->routeIs('admin.products.index') ? 'bg-gray-700' : '' }}">
                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z"></path>
                </svg>
                <span class="mx-4">Daftar Produk</span>
            </a>
            <a href="{{ route('admin.users.index') }}"
                class="flex items-center px-4 py-2 mt-2 rounded-md hover:bg-gray-700 {{ request()->routeIs('admin.users.index') ? 'bg-gray-700' : '' }}">
                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <span class="mx-4">Daftar Pengguna</span>
            </a>
        @endif

        <!-- Menu Khusus Admin (lanjutan) -->
        @if (auth()->user()->role == 'superadmin')
            <a href="{{ route('admin.users.index') }}"
            ...
            <a href="{{ route('admin.settings.index') }}"
                ...
                <span class="mx-4">Pengaturan Website</span>
            </a>
        @endif
    </nav>
</aside>
