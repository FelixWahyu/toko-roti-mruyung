    ...
    <table class="min-w-full divide-y divide-gray-200 text-sm">
        <thead class="bg-gray-100">
            <tr>
                ...
                <th class="px-5 py-3 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider">Total</th>
        ...
        <tbody class="bg-white divide-y divide-gray-200">
            @forelse($orders as $order)
                <tr class="hover:bg-gray-50/70 transition-colors">
                    <td class="px-5 py-3 whitespace-nowrap font-bold text-gray-900">{{ $order->order_code }}</td>
                    <td class="px-5 py-3 whitespace-nowrap text-gray-700">{{ $order->user->name }}</td>
                    <td class="px-5 py-3 whitespace-nowrap text-gray-500">{{ $order->created_at->format('d M Y') }}</td>
                    <td class="px-5 py-3 whitespace-nowrap font-medium text-gray-900">Rp{{ number_format($order->grand_total, 0, ',', '.') }}</td>
                    <td class="px-5 py-3 whitespace-nowrap">
                        <span
                            class="inline-flex items-center space-x-1.5 px-2 py-1 text-xs font-semibold rounded-sm capitalize
                    ...
                    <td class="px-5 py-3 whitespace-nowrap text-right text-sm font-medium">
                        <a href="{{ route('admin.orders.show', $order) }}"
                            class="inline-flex items-center space-x-1.5 bg-gray-100 hover:bg-gray-200 text-gray-800 font-semibold py-1.5 px-3 rounded-sm text-xs transition">
                            <span>Detail</span>
                            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"
            ...
            @empty
                <tr>
                    <td colspan="7" class="px-6 py-6 text-center text-xs text-gray-500">Tidak ada data pesanan.</td>
                </tr>
            @endforelse
        </tbody>
    </table>
